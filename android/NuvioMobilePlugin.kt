...
class NuvioMobilePlugin(private val host: Activity) : Plugin(host) {
    ...
    @Command fun updateSyncNotification(invoke: Invoke) {
        ...
        val intent = Intent(if (args.active) NuvioForegroundService.ACTION_UPDATE_SYNC else NuvioForegroundService.ACTION_STOP_SYNC)
            .putExtra("scanned", args.scanned).putExtra("phase", args.phase).putExtra("error", args.error)
        args.total?.let { intent.putExtra("total", it) }
    ...
    @Command fun updateUploadNotification(invoke: Invoke) {
        ...
        val intent = Intent(if (args.active) NuvioForegroundService.ACTION_UPDATE_UPLOAD else NuvioForegroundService.ACTION_STOP_UPLOAD)
            .putExtra("total", args.total).putExtra("completed", args.completed).putExtra("pending", args.pending)
            .putExtra("failed", args.failed).putExtra("paused", args.paused).putExtra("cancelled", args.cancelled)
            .putExtra("processedBytes", args.processedBytes).putExtra("totalBytes", args.totalBytes)
            .putExtra("speedBps", args.speedBps).putExtra("currentFileName", args.currentFileName).putExtra("phase", args.phase)
        args.percent?.let { intent.putExtra("percent", it) }
        args.etaSeconds?.let { intent.putExtra("etaSeconds", it) }
        deliverNotification(invoke, intent)
    }
