...
$env:NUVIO_ANDROID_KEYSTORE = Join-Path $project '.release-signing/nuvio-android.jks'
$env:NUVIO_ANDROID_KEY_PASSWORD = (Get-Content -LiteralPath (Join-Path $project '.release-signing/android-password.txt') -Raw).Trim()
try {
    ...
    $result
    if ($LASTEXITCODE -ne 0 -or ($result -join "`n") -notmatch 'NUVIO_NATIVE_TESTS_PASSED') { throw 'La prueba nativa de Android no pasó.' }
} finally {
